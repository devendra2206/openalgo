...
# ATM-lock background dispatch, PnL push, the trade-log writer, plus
...
# thread" (2026-07-28, on the Combined script specifically, but the same
# risk applies here). Must be called before any thread is created; affects
# every threading.Thread from here on, including ones spawned internally
    ...
    def load(self):
        ...
        for inst in INSTRUMENTS:
            ...
            self.state.instrument_locks[inst.name] = InstrumentLock(
                **{**asdict(InstrumentLock()), **filter_known_fields(InstrumentLock, lock_raw)}
            )
...
def fetch_symbol_ltp(client, symbol: str, exchange: str, require_two_sided: bool = False) -> Optional[float]:
    ...
    2026-08-10 on this same broker (this strategy's own SENSEX PE leg,
    SENSEX13AUG2678600PE): a MARKET order's MPP-protected LIMIT price came
    back computed off ltp=78525.19 (the underlying SENSEX INDEX's own spot
    level, not the option's ~450 premium) with bid=0/ask=0, rejected by the
    broker but leaving the leg unentered for 46 minutes. Pass True only for
...
class StrategyEngine:
                ...
                for option_type in ("PE", "CE"):
                    ...
                    if ltp is not None:
                        ...
                        if due:
                            ...
                            rest_ltp = fetch_symbol_ltp(self.ltp_client, pos.symbol, inst.options_exchange, require_two_sided=True)
    ...
    def _get_option_signal(self, leg_key: str, symbol: str, exchange: str,
        ...
        if cached is None:
            return None
        if ltp is None:
            ltp = fetch_symbol_ltp(self.ltp_client, symbol, exchange, require_two_sided=True)
        if ltp is not None:
            cached.ltp = ltp
        return cached
            ...
            for inst in INSTRUMENTS:
                ...
                for option_type in ("PE", "CE"):
                    ...
                    if opt_ltp is None:
                        ...
                        opt_ltp = fetch_symbol_ltp(self.ltp_client, symbol, inst.options_exchange, require_two_sided=True)
                    else:
