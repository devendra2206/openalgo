"""
===============================================================================
NIFTY Trend-Following -- Dual-Timeframe (1 Day/1hr), Synthetic-Combo Execution
===============================================================================
Version     : 1.0.0
Platform    : OpenAlgo Hosted Strategy
OpenAlgo    : >= 2.0.1.5
Python      : >= 3.11
Derived from: `MCX_CrudeOil_EMA9_RSI_Intraday_1_20260723150000.py` for all
              platform-integration scaffolding (Environment/Broker/
              PriceStream/StateStore, order lifecycle, error recovery, PnL
              reporting, WhatsApp self-alert) -- see AUTHORING_CHECKLIST.md
              reference #1. The SIGNAL/EXECUTION model is entirely new, not
              copied from any existing script (see below).
...
timeframes per instance:
  - Big timeframe:   1 trading day, aggregated LOCALLY from broker-native
                      "1h" bars (grouped by each bar's own correct IST
                      calendar date) -- NEVER fetched via
                      client.history(interval="D") directly. Confirmed
                      2026-08-17: the installed openalgo SDK mislabels
                      every daily bar's date one day early for that
                      interval (see Config.big_fetch_interval and
                      compute_timeframe_signal's docstrings for the full
                      root-cause trace -- it's a bug in the pip-installed
                      SDK, not this repo or the server). A trading session
                      is ~375 minutes, far short of 1440, so the usual
                      09:15-anchored intraday resampler (_resample_to_
                      candle_interval) is meaningless for daily bars too --
                      see _aggregate_to_daily instead.
  - Small timeframe: 1 hour (fetched via broker-native "1h" directly)
This is Instance A of two independently-deployed instances (see
Nifty_TrendFollow_DualTF45_15_1_*.py for Instance B, Big=45min/Small=15min --
same logic, different timeframe pair, fully separate state/positions/PnL).
NOTE: Instance A's Big/Small ratio does not match the source spec's "Big =
Small x 3" convention (1 day is much more than 3 hours) -- this pairing was
explicitly requested by the user as a second, independent deployment
alongside Instance B's 3x-ratio pair, not derived from the same rule.
...
Notes / Assumptions (please verify against your installed `openalgo` SDK):
  ...
  * Confirmed live (2026-08-17) against `broker/shoonya/api/data.py`'s
    ...
    4h/D. "1h" is used directly for BOTH Big and Small here -- Small
    consumes it as-is, Big aggregates it locally to daily bars via
    `_aggregate_to_daily`. `config.big_fetch_interval` and `small_fetch_
    interval` are both "1h"; `interval_minutes >= 1440` inside
    `compute_timeframe_signal` routes to `_aggregate_to_daily` BEFORE
    `_fetch_interval_matches_target` is even consulted.
  * Deliberately NEVER fetches `interval="D"` directly. Confirmed live
    (2026-08-17) against the real algodev.co.in API: the installed
    openalgo SDK (pip package `openalgo==2.0.3` -- the current latest
    PyPI release, so there is no newer version to pin instead) mislabels
    every daily bar's date one day EARLY when fetched via
    `client.history(interval="D")`. Root cause, verified by directly
    reading the SDK source: `.venv/Lib/site-packages/openalgo/data.py:
    284-289` does `pd.to_datetime(timestamp, unit="s")` then explicitly
    SKIPS the UTC->IST `tz_localize`/`tz_convert` step for
    `interval in ('D','W','M')`, based on a false "daily data is already
    in IST" assumption in its own docstring -- the epoch is genuinely UTC,
    so skipping that conversion shifts the naive datetime to `18:30 the
    previous day`, and `.date()` of that reads one calendar day early.
    Verified this is NOT a bug in this repo: Shoonya's own broker-adapter
    (`broker/shoonya/api/data.py:_get_daily_from_intraday`, which
    aggregates daily bars from 5m candles rather than trusting Shoonya's
    unreliable native daily endpoint -- see that file's own comment,
    ~line 661) computes a genuinely correct epoch, re-checked line by
    line; and the raw OpenAlgo server API response is correct too --
    only `client.history(interval="D")` through this exact SDK version
    mislabels it. `_aggregate_to_daily` sidesteps the bug entirely by
    never calling that code path. history_lookback_days is raised to 120
    for this instance (vs Instance B's 10) specifically so the Big=Daily
    aggregation clears the MACD(9,23,7)+SuperTrend(8,3.2) warmup floor
    (~30 daily bars) after accounting for weekends/holidays -- 10
    calendar days would return well under 10 trading days, nowhere near
    enough.
...
@dataclass
class Config:
    strategy_name: str = "NIFTY Trend-Follow Dual-TF (1 Day/1hr) Synthetic-Combo"
    version: str = "1.0.0"
    instance_id: str = "1D_1hr"    # distinguishes Instance A from Instance B in logs/state file names

    # --- Dual timeframe -------------------------------------------------
    big_timeframe_minutes: int = 1440   # 1 trading day
    small_timeframe_minutes: int = 60   # 1 hour -- NOT Big/3 for this instance, see module docstring
    # Broker-native interval to FETCH for each timeframe. Both Big and
    # Small fetch "1h" (natively supported by Shoonya) -- Small uses it
    # directly, Big aggregates it LOCALLY up to one row per trading day via
    # _aggregate_to_daily. Deliberately NEVER "D": confirmed 2026-08-17 that
    # the installed openalgo SDK (pip package openalgo==2.0.3, the current
    # latest PyPI release) mislabels every daily bar's date one day EARLY
    # when fetched via client.history(interval="D") -- see
    # compute_timeframe_signal's docstring for the full root-cause trace.
    # This is a bug in the pip-installed SDK itself, not in this repo's
    # broker-adapter code or the OpenAlgo server's API response.
    big_fetch_interval: str = "1h"
    small_fetch_interval: str = "1h"
    history_lookback_days: int = 120    # raised from Instance B's 10 -- the Big=Daily fetch needs
                                         # ~30 trading-day bars to clear MACD/SuperTrend warmup;
                                         # 120 calendar days comfortably covers that after
                                         # weekends/holidays. Harmless for the Small=1hr fetch too.
...
def _aggregate_to_daily(bars):
    """Aggregate intraday (native '1h', tz-correct) bars up to one row per
    trading day -- deliberately built LOCALLY instead of ever calling
    client.history(interval="D"). Confirmed 2026-08-17, live, against the
    real algodev.co.in API: the installed openalgo SDK (pip package
    openalgo==2.0.3, .venv/Lib/site-packages/openalgo/data.py:284-289)
    skips its UTC->IST timezone conversion specifically for interval in
    ('D','W','M') -- based on a false "daily data is already in IST"
    assumption in its own docstring -- which silently mislabels every
    daily bar's date one day EARLY (verified by cross-checking against
    unambiguous 15m-derived daily OHLC on multiple real trading days: the
    row timestamped e.g. "2026-08-04" actually contains 2026-08-05's real
    open/high/low/close). This is a bug in the pip-installed third-party
    SDK itself (2.0.3 is the current latest release on PyPI, so there is
    no newer version to pin), not in this repo's broker-adapter code
    (Shoonya's own broker/shoonya/api/data.py:_get_daily_from_intraday
    computes a genuinely correct epoch -- re-verified line by line) or in
    the OpenAlgo server's API response. `bars` must already be tz-aware in
    Asia/Kolkata (true of every INTRADAY interval, since the SDK's
    tz_localize/tz_convert step is only skipped for 'D'/'W'/'M') --
    grouping by each bar's own correct IST calendar date and taking
    open=first/high=max/low=min/close=last/volume=sum per day reproduces a
    correct daily OHLC with no dependency on the broken endpoint at all."""
    if not len(bars):
        return bars
    ...
    # Re-anchor each day's index to that day's session open (09:15 IST) --
    # matches how every other candle_key in this file is expressed (a
    # session-open timestamp, not a bare date), so downstream comparisons
    # (candle_key equality, big_close sort keys) stay consistent in shape.
    daily.index = pd.to_datetime(daily.index).tz_localize(IST) + pd.Timedelta(hours=9, minutes=15)
    daily.index.name = bars.index.name
    return daily
...
class StateStore:
    ...
    def load(self):
        ...
        for simple_field in ("direction", "entry_time", "entry_futures_px", "controlling_timeframe",
                              ...
                              "sl_reference_low", "expiry_compact", "execution_id", "is_dry_run",
                              "closing"):
...
def _fetch_interval_matches_target(fetch_interval: str, interval_minutes: int) -> bool:
    ...
    resample needed). fetch_interval is NEVER 'D' here -- see
    compute_timeframe_signal's own docstring and _aggregate_to_daily for
    why Big=1 Day is built locally from '1h' bars instead of ever calling
    client.history(interval="D") (confirmed SDK bug, 2026-08-17)."""
...
def compute_timeframe_signal(client, inst: InstrumentConfig, futures_symbol: str,
    ...
    futures contract, then either:
      - aggregate LOCALLY to one row per trading day (_aggregate_to_daily)
        when interval_minutes>=1440 (Big=1 Day) -- NEVER via
        client.history(interval="D") directly. Confirmed 2026-08-17, live
        against algodev.co.in: the installed openalgo SDK (pip package
        openalgo==2.0.3, the current latest PyPI release -- there is no
        newer version to pin) mislabels every 'D'/'W'/'M' bar's date one
        day EARLY (.venv/Lib/site-packages/openalgo/data.py:284-289 skips
        its UTC->IST conversion specifically for those intervals, based on
        a false "daily data is already in IST" assumption). Verified this
        is a bug in the pip-installed SDK itself, not in this repo's
        Shoonya broker-adapter code (broker/shoonya/api/data.py:
        _get_daily_from_intraday computes a genuinely correct epoch) or in
        the OpenAlgo server's API response -- the raw REST API and any
        non-SDK caller are unaffected; only client.history(interval="D")
        through this exact SDK version is.
      - resample to interval_minutes ONLY if fetch_interval isn't already
        that granularity (see _fetch_interval_matches_target), for every
        other (intraday) target.
    Then compute MACD(9,23,7) + SuperTrend(8,3.2) off the last genuinely
    CLOSED bar. Returns None (logged) if anything is unavailable."""
    ...
    if interval_minutes >= 1440:
        bars = _aggregate_to_daily(raw_bars)
    elif _fetch_interval_matches_target(fetch_interval, interval_minutes):
        bars = raw_bars
    else:
        bars = _resample_to_candle_interval(raw_bars, interval_minutes)
    if bars.empty:
...
def _correct_breached_sl_reference(direction: str, entry_futures_px: float,
    ...
    3.2-year backtest (2026-08-20) on the 27m/9m sibling instance: 105 of
    ~2196 fresh entries had the candle reference already on the WRONG side
    of the live entry price (e.g. a LONG's sl_reference_low sitting ABOVE
    its own entry price) -- the trade is technically born already past its
    own candle-break stop.
    ...
    while True:
        ...
        try:
            ...
            (strategy_tag, execution_id, direction, role, symbol, action, quantity,
             entry_time, entry_px, exit_time, exit_px, exit_reason, is_dry_run) = item
...
class StrategyEngine:
    ...
    #     intrabar LTP. Especially significant for Instance A: Big=Daily
    #     only "closes" once per day, so the candle-break mechanism for a
    #     BIG-controlled position effectively goes dormant intraday and
    #     only re-checks at the next daily close -- the 1% mechanism is
    #     what actually protects against an intraday move in the meantime.
    # -------------------------------------------------------------------
    ...
    # just closed, never continuous/intrabar).
    # -------------------------------------------------------------------
    ...
    def _check_sl_reentry(self, futures_px: float) -> bool:
        ...
        if not recovered:
            ...
            # it let a still-bullish Big/Daily signal re-enter LONG
            # immediately at a WORSE price than the original entry, no
            # recovery at all -- especially severe for Instance A since a
            # Daily Big signal is static for the whole day).
            return True
        ...
        for role in close_order:
            ...
            if is_dry:
                ...
                append_trade_log(self.env.strategy_tag, pos.execution_id, pos.direction, role,
                                 ...
                                 exit_time, leg.exit_px, reason, True)
            elif wait_for_fills:
    ...
    def _watch_exit_fill(self, role: str, leg: "OptionLeg", close_action: str,
        ...
        try:
            ...
            append_trade_log(self.env.strategy_tag, pos.execution_id, pos.direction, role,
                             leg.symbol, leg.action, leg.quantity, pos.entry_time, leg.entry_px,
                             exit_time, exit_px, reason, False)
    ...
    def run_cycle(self):
        try:
            ...
            # must never go blind for the last stretch of trading each day.
            # Corrected 2026-08-18 (user correction) after this exact gate
            # pushed a Big-candle signal-loss exit from the same day it
            # happened to the next morning -- confirmed via a real trade in
            # backtest.
