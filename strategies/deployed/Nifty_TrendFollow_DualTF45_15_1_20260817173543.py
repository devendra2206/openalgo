"""
===============================================================================
NIFTY Trend-Following -- Dual-Timeframe (45m/15m), Synthetic-Combo Execution
===============================================================================
Version     : 1.0.0
...
timeframes per instance:
  - Big timeframe:   45 minutes
  - Small timeframe: 15 minutes (= Big / 3, the source spec's own ratio)
This is Instance B of two independently-deployed instances (see
Nifty_TrendFollow_DualTF1D_1hr_1_*.py for Instance A, Big=1 Day/Small=1hr --
...
Notes / Assumptions (please verify against your installed `openalgo` SDK):
  ...
  * Confirmed live (2026-08-17) against `broker/shoonya/api/data.py`'s
    ...
    4h/D only -- NO native 45m, same gap CrudeOil hit at 9m. This instance
    fetches broker-native `config.big_fetch_interval=config.small_fetch_
    interval="15m"` (natively supported, used directly as the Small
    timeframe) and locally resamples to 45m for the Big timeframe via
    `_resample_to_candle_interval` -- same pattern as CrudeOil's 9m fix.
    Do not set big_fetch_interval="45m" directly; it will raise
    "Unsupported interval" at the broker call. Instance A (Big=1 Day/
    Small=1hr) fetches "D"/"1h" directly instead -- see
    compute_timeframe_signal's `_fetch_interval_matches_target` guard,
    which is what makes fetch_interval genuinely independent per timeframe.
...
@dataclass
class Config:
    strategy_name: str = "NIFTY Trend-Follow Dual-TF (45m/15m) Synthetic-Combo"
    version: str = "1.0.0"
    instance_id: str = "45m_15m"   # distinguishes Instance B from Instance A in logs/state file names

    # --- Dual timeframe -------------------------------------------------
    big_timeframe_minutes: int = 45
    small_timeframe_minutes: int = 15   # Big / 3, per the source spec's own ratio
    ...
    # (Big=45m/Small=15m), Shoonya has no native 45m (confirmed 2026-08-17
    # against broker/shoonya/api/data.py's timeframe_map), so Big resamples
    # from the same native 15m fetch Small already uses directly.
    big_fetch_interval: str = "15m"
    small_fetch_interval: str = "15m"
...
def _resample_to_candle_interval(bars, interval_minutes: int):
    ...
    is ~10 calendar days stale. Harmless here since 45 and 15 both evenly
    divide 1440 (minutes/day), so a stale-day origin still lands on the
    same daily grid -- but the 27m/9m sibling instance confirmed live
    2026-08-20 that an interval NOT evenly dividing 1440 drifts the whole
    day's grid by (1440 mod interval) minutes per elapsed day when anchored
    to a stale index[0] day. Fixed here too to keep this function identical
    across all three instances and remove the latent risk entirely."""
...
class StrategyEngine:
    ...
    def _handle_rollover(self):
        ...
        if ctx["futures_symbol"] != prior_futures_symbol:
            # WS subscription for the futures symbol -- without this,
            # run_cycle's price_stream.get_ltp() for the futures price would
            # ALWAYS return None (never-subscribed cache), silently falling
            # through to a REST fetch_symbol_ltp() call every single cycle.
    ...
    def _open_position(self, direction: str, timeframe: str, futures_px: float,
        ...
        # and any other LTP read against these symbols would ALWAYS miss the
        # price_stream cache and fall back to a REST quotes() call every time.
        self.price_stream.add_instruments([
    ...
    def _close_position(self, reason: str, wait_for_fills: bool = False) -> bool:
        ...
        for role in close_order:
            ...
            # WS-first -- leg has been subscribed since entry, likely populated.
            ltp = self.price_stream.get_ltp(leg.symbol, self.inst.options_exchange,
                                            _current_ws_stale_threshold())
            if ltp is None:
                ltp = fetch_symbol_ltp(self.client, leg.symbol, self.inst.options_exchange)
        ...
        if pos.direction:
            for role in _LEG_ROLES:
                ...
                if leg.symbol and leg.entry_filled and not leg.exit_filled:
                    # WS-first, REST only as fallback -- previously called REST
                    # unconditionally here every config.pnl_tick_interval=0.8s
                    # per open leg, a literal "per-tick REST poll."
                    ltp = self.price_stream.get_ltp(leg.symbol, self.inst.options_exchange,
                                                    _current_ws_stale_threshold())
                    if ltp is None:
                        ltp = fetch_symbol_ltp(self.client, leg.symbol, self.inst.options_exchange)
