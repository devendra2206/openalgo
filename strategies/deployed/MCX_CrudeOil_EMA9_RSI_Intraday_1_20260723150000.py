...
class PriceStream:
    ...
    def _watchdog_loop(self):
        ...
        while not self._stop.is_set():
            ...
            # investigation): [MCX] tag + before/after markers on the
            # unsubscribe/subscribe pair, to correlate against
            # Combined/Batman's matching tags and websocket_proxy/server.py's
            # DEBUG-TEMP logs -- if two strategies' calls for the SAME shared
            # symbol land within the same second, that confirms the suspected
            # subscription_index refcount race. Remove once root cause is
            # confirmed/fixed.
            Log.warning(f"[DEBUG-TEMP][MCX] about to unsubscribe_ltp: {due_names}")
            try:
                self.client.unsubscribe_ltp(due_for_retry)
                Log.warning(f"[DEBUG-TEMP][MCX] unsubscribe_ltp returned: {due_names}")
            except Exception as exc:
