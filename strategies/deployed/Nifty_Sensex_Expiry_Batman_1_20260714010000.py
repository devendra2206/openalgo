...
class PriceStream:
    ...
    def _watchdog_loop(self):
        ...
        while not self._stop.is_set():
            ...
            # investigation): [BATMAN] tag + before/after markers on the
            # unsubscribe/subscribe pair, to correlate against
            # Combined/MCX's matching tags and websocket_proxy/server.py's
            # DEBUG-TEMP logs -- if two strategies' calls for the SAME shared
            # symbol (e.g. NIFTY.NSE_INDEX) land within the same second, that
            # confirms the suspected subscription_index refcount race. Remove
            # once root cause is confirmed/fixed.
            Log.warning(f"[DEBUG-TEMP][BATMAN] about to unsubscribe_ltp: {due_names}")
            try:
                self.client.unsubscribe_ltp(due_for_retry)
                Log.warning(f"[DEBUG-TEMP][BATMAN] unsubscribe_ltp returned: {due_names}")
            except Exception as exc:
