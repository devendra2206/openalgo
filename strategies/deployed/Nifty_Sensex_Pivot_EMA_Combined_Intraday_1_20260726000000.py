...
# Python's default thread stack reservation is 8MB. This process runs ~12
# threads at once (8 fill-watchers, 2 signal-refresh, 1 PnL push, the
...
# near deep recursion. At the default size that's ~96MB of virtual address
# space reserved purely for stacks, out of the STRATEGY_MEMORY_LIMIT_MB
...
# thread" (2026-07-28). Must be called before any thread is created;
# affects every threading.Thread from here on, including ones spawned
# internally by ThreadPoolExecutor.
...
try:
    ...
    # is affected. A live trading process failing to even START over a
    # missing notification helper would be a far worse outcome than losing
    # one dashboard convenience.
...
class LegPosition:
    ...
    entry_px: float = 0.0          # Set from a pre-trade LTP snapshot when the order is placed,
                                    ...
                                    # _watch_entry_fill) -- the exit decision itself never looks
                                    # at price, only the trade log's PnL columns do.
    entry_order_id: str = ""
...
class Broker:
    ...
    def connect(self):
        ...
        self.client = api(
            ...
            # resubscribe on this same client (see class docstring below).
            # Leaving the SDK's own auto_reconnect thread enabled races it:
            # both call _do_connect() on self.ws independently, which can
            # tear down/replace the socket concurrently and immediately
            # trigger another spurious close -- observed in production as a
            # ~45-50s repeating "connection down" cycle that never settles.
            # The watchdog is the single owner of reconnect for this client.
...
class PriceStream:
    def __init__(self, client, instruments: list):
        ...
        # different symbol that just started going stale. See
        # _watchdog_loop's docstring for the full rationale.
    ...
    def _watchdog_loop(self):
        ...
        1. Connection-down (transport itself dead) -> full reconnect
           immediately -- nothing per-symbol can fix a dead socket.
        2. Connection alive, but some symbols' ticks are stale -> per-symbol
           resubscribe, each on its OWN independent backoff
           (_symbol_backoff_step/_symbol_next_retry_at) so one chronically
           stale symbol's growing wait never slows down retries for a
           DIFFERENT symbol. Only escalates to a full reconnect if a
           MAJORITY of tracked symbols are simultaneously stuck past
           ws_stale_reconnect_after consecutive cycles (a real
           connection-wide signal) -- AND only after
           _confirm_genuinely_broken_via_rest() shows real price movement
           the WS feed isn't delivering, not just thin liquidity. Confirmed
           in production, 2026-07-29: a single thinly-traded MCX option leg
           stayed stale for an extended stretch while the futures contract
           on the SAME connection ticked fine the whole time -- the OLD
           "any one symbol escalates" rule would have (and did) force
           repeated full reconnects that could never fix a liquidity
           problem, disrupting the healthy futures stream for nothing.
        """
...
class StrategyEngine:
    def __init__(self, client, store: StateStore, env: Environment, price_stream: "PriceStream",
        ...
        # config.pnl_rest_fallback_interval_sec's docstring.
    ...
    def get_signal(self, inst: InstrumentConfig, ltp: Optional[float] = None,
        ...
        (feeding BOTH engines) refresh once the cached signal's own candle
        falls behind the current wall-clock candle boundary (see
        _current_candle_boundary()'s docstring) -- NOT on a rolling timer,
        which would keep re-fetching the same still-open candle ~12 times
        per 3-minute window for no benefit. LTP is refreshed on every call.
    ...
    def report_pnl_tick(self):
        """Runs on its OWN scheduler job (0.8s), reading the WS price cache
        first, falling back to a THROTTLED REST quotes() call (at most once
        per config.pnl_rest_fallback_interval_sec per leg) once the WS
        ...
        best-known price rather than disappearing outright. Sums PnL across
        ALL 8 legs (both engines) into one combined push -- see
        report_pnl_to_platform's docstring."""
        try:
            open_positions = []
            for leg_key in LEG_KEYS:
                ...
                if current_px is not None:
                    self._pnl_last_known_price[leg_key] = current_px
                else:
                    now = datetime.now(IST)
                    ...
                    if due:
                        ...
                        rest_px = fetch_symbol_ltp(self.ltp_client, pos.symbol, inst.options_exchange, require_two_sided=True)
                        if rest_px is not None:
                            self._pnl_last_known_price[leg_key] = rest_px
                    current_px = self._pnl_last_known_price.get(leg_key)
    ...
    def _refresh_pending_action_bg(self, leg_key: str):
        """Dispatches check_pending_action to _signal_executor instead of
        blocking run_cycle() -- mirrors this file's own _refresh_force_
        exit_check_bg's use of _signal_executor, but keyed per leg_key
        (multiple legs can be in error state at once) and caches the
        result for _pop_pending_action() to consume, since (unlike
        push_leg_error/notify_trade_closed) the caller needs the return
        value, not just fire-and-forget. Guarded per leg_key so a slow
        check that outlives one cycle isn't resubmitted on top of itself."""
    ...
    def _finalize_exit(self, leg_key: str, inst: InstrumentConfig, reason: str):
        ...
        # fill there; (2) the broker's REAL average fill price, captured by
    ...
    def run_cycle(self):
        try:
            ...
            # already routed through _enter_error_mode's own alert or the
            # per-instrument isolation except above. Throttled since an
            # outer catch-all could otherwise fire every scheduler tick if
