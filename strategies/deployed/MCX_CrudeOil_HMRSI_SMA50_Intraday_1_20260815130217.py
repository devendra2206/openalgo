"""
===============================================================================
MCX CRUDEOIL HM(WMA-of-RSI)/RSI/SMA50 Intraday Option Buyer (CE/PE)
===============================================================================
Version     : 1.0.0
Platform    : OpenAlgo Hosted Strategy
OpenAlgo    : >= 2.0.1.5
Python      : >= 3.11
Derived from: `MCX_CrudeOil_EMA34_RSI_ADX_Intraday_1_20260811103000.py` --
              same platform-integration skeleton verbatim (non-blocking
              run_cycle, retry/cancel/manual error recovery, state
              persistence, dynamic PriceStream, background order-fill
              polling, futures/options expiry roll coupling), BUY-side
              (long-option, bounded-risk) order/PnL convention unchanged.
              Only the signal model, stop-loss mechanism, and entry/exit
              state machine are new. User-provided spec, built 2026-08-15.
...
premium paid, no margin/undefined-risk exposure. UNLIKE the EMA34/RSI/ADX
sibling, this strategy DOES have a native per-trade stop-loss (the
"momentum candle" price level, see below) -- but it is defined on the
UNDERLYING FUTURES price, not the option's own premium, so a fast enough
premium move could still outrun it between checks.

Source spec summary (external material, not this project's own design):
------------------------------------------------------------------------
  RSI_raw = RSI(close, period=9)
  HM_line = WMA(RSI_raw, period=21)     # the "red line"
  SMA50   = SMA(close, period=50)       # price-trend filter
  THRESHOLD = 50                        # replaces standard 30/70 levels

  SHORT/PE setup: HM_line > RSI_raw AND HM_line > THRESHOLD marks
  "extended_up" and arms a wait-for-reversal watch. Once HM_line crosses
  BELOW THRESHOLD, a sustain counter starts (must hold N_SUSTAIN_CANDLES
  candles, not fire on the first tick past the cross -- filters noise).
  Once sustained AND price closes below SMA50 on TWO consecutive candles,
  a PE is bought. Stop-loss = the high of the "momentum candle" -- the
  larger, decisive candle that kicked off the down-move (see
  `find_momentum_candle` below for the objective proxy rule used here,
  since the source only describes this visually). Exit is a TRAILING
  condition, not the fixed stop alone: close crossing back above SMA50
  also closes the position. LONG/CE setup is the exact mirror.

  Re-entry: the source explicitly allows MULTIPLE re-entries per session.
  After an exit, a fresh entry is allowed once EITHER price returns to
  touch the SMA50 zone, OR HM_line crosses THRESHOLD again in the trade
  direction -- whichever happens first.

Parameters the source spec left UNSPECIFIED -- shipped with documented
defaults below, tunable via Config, marked "needs backtesting":
  - N_SUSTAIN_CANDLES (source: "wait until it sustains", no number given)
  - Momentum-candle detection threshold/window (source: visual description
    only -- "the larger, decisive candle")
  - SMA50 "touch zone" tolerance for the re-entry gate (not addressed)
  Per explicit user decision, these are NOT blocking implementation --
  reasonable defaults ship now, tune from live results later.
...
candles (15-minute, per the source spec's own "recommended floor"):
  - RSI(9) on Close ("RSI_raw"), WMA(21) of RSI_raw ("HM_line", the "red
    line"), SMA(50) on Close -- all computed over the last-closed
    candle_interval_minutes bars (the newest fetched bar is unconditionally
    dropped as still-forming, same defensive pattern used everywhere else
    in this project). No VWAP/EMA/ADX in this signal -- unlike the
    EMA34/RSI/ADX sibling, this spec doesn't use them.
  - Both entry AND exit are evaluated purely off the last CLOSED candle's
    own values -- no live-LTP breakout-confirmation term (the spec doesn't
    call for one; the sustain-counter + false-signal-filter machinery below
    already serves a similar noise-filtering purpose).

  PE (SHORT) setup, per leg, tracked in LegState (persists across
  restarts):
    extended_up   : HM_line[-1] > RSI_raw[-1] AND HM_line[-1] > THRESHOLD(50)
                     -> arms wait_for_reversal, resets sustain_counter
    crossed_below : wait_for_reversal AND HM_line crosses below THRESHOLD
                     -> anchors signal_candle_high/low/key to THIS candle,
                        runs find_momentum_candle() to lock in the SL level,
                        starts sustain_counter at 1
    sustaining    : while HM_line stays below THRESHOLD, sustain_counter
                     increments each candle
    false-signal  : if HM_line reverses back above THRESHOLD before
                     signal_confirmed_by_close is True, the whole episode
                     resets (source: "only trust the cross once price
                     closes beyond the high/low of the candle at the time
                     of the crossing" -- signal_confirmed_by_close is set
                     True the first candle close_prev1 < signal_candle_low)
    entry         : sustain_counter >= config.n_sustain_candles
                     AND signal_confirmed_by_close
                     AND close_prev1 < SMA50_prev1 AND close_prev2 < SMA50_prev2
                        (source's explicit 2-close confirmation)
                     AND a momentum candle was found (momentum_candle_high > 0)
                     -> BUY PE, stop_loss = momentum_candle_high, reset
                        wait_for_reversal/sustain_counter

  CE (LONG) setup is the exact mirror (HM_line < RSI_raw AND < THRESHOLD,
  crosses above, close > SMA50 x2, stop = momentum_candle_low).

  Exit (either one closes the leg, checked every closed candle -- neither
  is a live-tick check, unlike the EMA34/RSI/ADX sibling's %-based SL).
  2026-08-15: trails EMA(trail_ma_period=20), NOT SMA50 -- deliberate
  deviation from the source spec's own exit rule (user-requested, after
  real-data backtesting on 1-7 Aug 2026 showed SMA50 lags badly on fast
  reversals, giving back most of a move before the 50-period average
  caught up). SMA50 itself is untouched everywhere else (entry
  confirmation, re-entry touch-zone gate):
    PE: close_prev1 > EMA20_prev1 (trailing EMA20-cross)
        OR underlying futures price > momentum_candle_high (fixed stop)
    CE: close_prev1 < EMA20_prev1
        OR underlying futures price < momentum_candle_low
  ...
  Momentum candle (source describes this only visually -- "the larger,
  decisive candle that kicked off the move"; this project's objective
  proxy, see find_momentum_candle()): the most recent bar within a rolling
  config.momentum_candle_lookback_k (20) window ending at the CURRENT
  candle whose (high-low) range exceeds config.momentum_candle_multiplier
  (1.5x) times the average range of the config.momentum_candle_range_avg_window
  (20) bars immediately preceding that window. Re-searched on EVERY
  sustaining candle via _refresh_momentum_candle() (2026-08-15 fix, not
  just once at the crossing candle) -- real-data backtesting (1-7 Aug
  2026) caught a sustain streak running for ~a month before the separate
  SMA50 confirmation also lined up, during which a "lock once" design
  left the stop level over 450 points stale by the time entry actually
  fired. Re-running every candle keeps it temporally anchored to NOW; if
  nothing currently qualifies, momentum_candle_high/low reset to 0 and
  entry is SKIPPED entirely (no synthetic fallback stop) -- an options
  buyer entering with no valid SL reference is a worse failure mode than
  missing the trade. Both the multiplier and the lookback window are
  **not specified by the source spec** -- shipped as documented defaults,
  needs backtesting.

  Stop-loss is a FIXED underlying-futures price level (not a %-of-premium
  ratchet like the EMA34/RSI/ADX sibling's compute_trailing_sl_price --
  that function is NOT used here) -- checked every closed candle against
  the futures LTP, translated into a SELL on the option leg exactly like
  any other exit reason via the same _exit_leg() path. A stop-loss/SMA50
  exit also blocks a fresh entry on THAT SAME leg until the wall-clock
  candle actually advances (LegState.last_sl_exit_candle_boundary, reused
  verbatim from the sibling), separate from the re-entry-trigger gate
  below.

  Re-entry (source explicitly allows MULTIPLE re-entries per session):
  after any exit, leg.awaiting_reentry_trigger is set. It clears -- letting
  the normal entry_condition run again -- once EITHER a fresh
  extended->crossed episode arms naturally, OR price touches the SMA50
  zone (abs(close-SMA50)/SMA50 <= config.sma50_touch_zone_pct, tolerance
  not specified by source, needs backtesting). config.max_trades_per_leg_per_day
  remains the outer session-wide cap bounding total re-entries.
...
------------------------------------------------------------------------
  ...
  - Max config.max_trades_per_leg_per_day (5) entries per leg per day.
...
base -- and the signal source (the futures contract, see above) must read
the SAME standard `CRUDEOIL` futures, not the Mini.

Notes / Assumptions (please verify against your installed `openalgo` SDK):
  * `ta.rsi(data, period)` -> ndarray. `ta.wma(data, period)` -> ndarray
    (confirmed present; no existing strategy in this repo used WMA before
    this one -- computed here on RSI_raw's own output, an indicator-of-an-
    indicator chain, which `ta.wma` doesn't care about). `ta.sma(data,
    period)` -> ndarray.
...
@dataclass
class Config:
    strategy_name: str = "MCX CRUDEOIL HM(WMA-of-RSI)/RSI/SMA50 Intraday Option Buyer"
    version: str = "1.0.0"

    # 2026-08-11 (sibling history, inherited): confirmed live the broker
    # rejects "45m" outright (HTTP 400 "Must be one of: ...15m, 20m,
    # 30m, 1h..." -- no 45m). Fetches at fetch_interval (15m,
    # broker-supported) and resamples to candle_interval_minutes-minute
    # bars locally in compute_instrument_signal. 15m is also the source
    # spec's own "recommended floor" timeframe, so both values are 15m --
    # the resample step is a 1:1 passthrough, kept as-is (not worth
    # special-casing away) since it still does real work if either value
    ...
    # warmup_needed (see compute_instrument_signal) is
    # max(rsi_period+wma_period, sma_period, momentum_candle_lookback_k+
    # momentum_candle_range_avg_window) + n_sustain_candles = 71 bars at
    # the defaults below -- at 15m that's ~17.75 market hours, i.e. a bit
    # over 1 MCX session (~14h55m). 5 calendar days leaves comfortable
    # margin for a weekend/short holiday cluster while still trimming the
    # per-call broker payload (same reasoning as the EMA34/RSI/ADX
    # sibling's own 10->5 day trim).
    history_lookback_days: int = 5

    # --- HM(WMA-of-RSI)/RSI/SMA50 signal, per the source spec ----------
    rsi_period: int = 9        # spec: RSI(close, period=9) -- "RSI_raw"
    wma_period: int = 21       # spec: WMA(RSI_raw, period=21) -- "HM_line"/red line
    sma_period: int = 50       # spec: SMA(close, period=50) -- price-trend filter
    threshold: float = 50.0    # spec: replaces the standard 30/70 oversold/overbought levels

    # NOT specified by the source spec ("wait until it sustains", no
    # number given) -- needs backtesting. Source suggests 2-3; 3 picked as
    # the more conservative default (fewer whipsaws).
    n_sustain_candles: int = 3

    # Momentum-candle detection -- source describes this only VISUALLY
    # ("the larger, decisive candle that kicked off the move"), no
    # objective rule given. This project's proxy (see find_momentum_candle):
    # a candle whose (high-low) range exceeds momentum_candle_multiplier x
    # the average range of the momentum_candle_range_avg_window bars
    # immediately preceding the search window. multiplier is the source's
    # own stated value; both window sizes are NOT specified -- needs
    # backtesting. momentum_candle_lookback_k and _range_avg_window default
    # to the same value (20, the source's own averaging window reused as
    # the search window too) but are kept as SEPARATE fields since they may
    # need to diverge once backtested.
    momentum_candle_lookback_k: int = 20
    # 2026-08-15: raising this 1.5 -> 2.0 was tried as an alternative to
    # the (reverted, see comment below) ADX gate, on the theory that a
    # more local change -- requiring a bigger breakout candle before the
    # state machine arms, rather than gating entries after the fact --
    # would avoid ADX's path-dependency problem. It didn't: also
    # backtested worse (1-15 Aug 2026: +Rs.2,588 -> -Rs.1,808, 13 trades,
    # 5W/8L). The momentum candle isn't just cosmetic -- it also SETS the
    # stop-loss level, so raising the multiplier changed which bar got
    # selected as the stop reference for surviving trades too, not just
    # which episodes got tracked; several trades that previously exited
    # via momentum_candle_stop shifted to a wider, more expensive
    # premium_15pct_stop instead (e.g. Aug 10 CE: -1020 -> -3786). Reverted
    # to 1.5. Left as a documented dead end -- do not re-try this lever in
    # isolation; any future attempt needs to separate "how selective is
    # entry" from "where does the stop get set" into two different knobs.
    momentum_candle_multiplier: float = 1.5
    momentum_candle_range_avg_window: int = 20

    # 2026-08-15: an ADX trend-strength entry gate (borrowed from the
    # EMA34/RSI/ADX sibling) was tried here after real-data backtesting
    # showed whipsaw losers on the entry side, then REVERTED after
    # backtesting the gate itself: over 1-15 Aug 2026 it cut trades from
    # 14 to 10 and PnL from +Rs.2,588 to -Rs.6,712. Root cause wasn't the
    # threshold value -- blocking an entry doesn't just remove a bad
    # trade, it leaves the leg flat so the state machine catches a
    # DIFFERENT, often worse, signal later (path-dependent), and the
    # sibling's adx_threshold=20.0 was never tuned against this HM/RSI/
    # SMA50 signal specifically. Not reintroduced without a longer
    # backtest window and its own threshold search.

    # Defines "price returns to the SMA50 zone" for the re-entry gate
    # (source: "if price returns to SMA50 zone... watch for fresh HM_line
    # cross to re-enter" -- exact tolerance NOT specified, needs
    # backtesting). abs(close - sma50) / sma50 <= this counts as a touch.
    # SMA50 is used ONLY for this re-entry-zone check and the entry's
    # 2-close confirmation now -- NOT for the exit condition, see
    # trail_ma_period below.
    sma50_touch_zone_pct: float = 0.001   # 0.1%

    # 2026-08-15: the exit condition (trailing close-vs-MA cross) was
    # originally SMA(50) per the source spec, but real-data backtesting
    # (1-7 Aug 2026) showed SMA50 lags badly on fast reversals -- e.g. a
    # CE entered right at a rally peak rode most of the way down before
    # the 50-period average finally caught up and exited, giving back the
    # bulk of what should have been a small, early loss. Replaced the
    # EXIT side only with a faster EMA(20) cross -- user-requested,
    # deliberate deviation from the source spec's own exit rule. Entry
    # still uses SMA50 (2-close confirmation) and the re-entry touch-zone
    # gate above, both unchanged -- only the trailing EXIT moved to this
    # tighter MA. NOT backtested against alternative periods yet.
...
class LegPosition:
    ...
    # reprice-and-cross-the-spread loop. (2026-08-18: found entry_px/exit
    # price here were NEVER corrected from poll_fill's real fill at all --
    # see docs/CUSTOMIZATIONS.md's "recurring gotcha" note.)
...
class LegState:
    ...
    # Inherited from the EMA34/RSI/ADX sibling (2026-08-11 incident there,
    # see that file's own comment for the full story): set to
    # _current_candle_boundary()'s WALL-CLOCK forming-candle boundary at
    # the moment ANY exit fires -- blocks a fresh entry on THIS SAME leg
    # (CE only blocks CE re-entry, PE is unaffected) until the candle
    # actually advances, so a just-exited trade can't immediately whipsaw
    # back in against the same stale-candle HM_line/RSI/SMA50 reading that
    # just closed it. Deliberately NOT InstrumentSignal.candle_key
    # (last-CLOSED candle from the cached signal) -- that cache refreshes
    # asynchronously in the background and can lag the true wall-clock
    # boundary by up to one refresh cycle; _current_candle_boundary() is
    # pure wall-clock (datetime.now() only, no cache/network dependency)
    # so it can't lag. Separate from awaiting_reentry_trigger below (both
    # gates must clear for a fresh entry).
    # Reset daily in _reset_day_if_needed alongside trade_count.
    last_sl_exit_candle_boundary: str = ""

    # --- HM(WMA-of-RSI)/RSI/SMA50 state machine, persisted so a restart
    # mid-sustain-count resumes correctly instead of silently dropping a
    # signal already partway through its sustain window. One instance per
    # leg (PE tracks the SHORT-side extended_up/crossed-below episode, CE
    # tracks the LONG-side mirror) -- see _update_hm_state_machine. ---
    wait_for_reversal: bool = False
    sustain_counter: int = 0
    signal_candle_high: float = 0.0     # anchors PE's false-signal-filter/momentum-candle search
    signal_candle_low: float = 0.0      # anchors CE's
    signal_candle_key: str = ""         # candle_key of the HM_line/threshold crossing candle
    signal_confirmed_by_close: bool = False   # false-signal filter: has price closed beyond signal_candle_high/low yet
    momentum_candle_high: float = 0.0   # locked-in PE stop-loss level (0.0 = not found/not applicable)
    momentum_candle_low: float = 0.0    # locked-in CE stop-loss level
    momentum_candle_key: str = ""       # which candle was identified as the momentum candle (diagnostic)
    # Set on any exit; cleared once EITHER a fresh extended->crossed episode
    # arms naturally OR price touches the SMA50 zone (see run_cycle) --
    # gates re-entry per the source spec's "multiple re-entries per
    # session" allowance, independent of last_sl_exit_candle_boundary's
    # same-candle whipsaw guard above (both must clear for a fresh entry).
    awaiting_reentry_trigger: bool = False
    # Reset daily in _reset_day_if_needed alongside trade_count/last_sl_exit_candle_boundary.
...
class InstrumentSignal:
    close_prev1: float
    close_prev2: float   # paired with sma50_prev2 for the spec's explicit 2-close SMA50 confirmation
    high_prev1: float
    low_prev1: float
    rsi_prev1: float          # RSI_raw at t-1
    hm_prev1: float            # WMA(RSI_raw, wma_period) at t-1 -- the "HM_line"/red line
    hm_prev2: float             # t-2, needed to detect a THRESHOLD cross at t-1
    sma50_prev1: float   # entry confirmation + re-entry touch-zone gate ONLY -- see trail_ma_prev1 for the exit
    sma50_prev2: float
    trail_ma_prev1: float   # EMA(trail_ma_period=20) at t-1 -- 2026-08-15, the actual EXIT reference (replaces SMA50 there)
    # Last (momentum_candle_lookback_k + momentum_candle_range_avg_window)
    # closed bars' highs/lows -- momentum-candle detection scans a window,
    # not a fixed offset, unlike the sibling's scalar adx_prev2/adx_prev3.
    high: np.ndarray
    low: np.ndarray
    ltp: float
    candle_key: str
    # True when `ltp` above is the closed-candle-close fallback (both WS
    # and REST live-price lookups failed), not a genuine live tick -- see
    # compute_instrument_signal's own comment. This strategy's entry/exit
    # conditions are candle-close-based, not live-tick-based (unlike the
    # EMA34/RSI/ADX sibling), so this flag currently has no consumer here
    # -- kept for parity with the shared InstrumentSignal shape convention
    # across this project's CrudeOil scripts, and in case a future revision
    # adds a live-tick term.
    ltp_is_fallback: bool = False


# Log the [inst] candle=... line only when the candle actually advances, not
# on every refetch (same fix applied to the other strategies in this
# project).
...
def compute_instrument_signal(client, inst: InstrumentConfig, futures_symbol: str,
    ...
    candle_interval_minutes (15m, 1:1 passthrough -- see
    _resample_to_candle_interval), then compute RSI(9) -> WMA(21) of that
    RSI ("HM_line") -> SMA(50) off the last genuinely CLOSED resampled
    bars, plus live LTP. Returns None (with a logged reason) if anything
    is unavailable."""
    ...
    # Warmup floor: RSI needs rsi_period bars; WMA(RSI) needs an ADDITIONAL
    # wma_period bars of already-valid RSI output (WMA computed on a
    # partially-warmed-up RSI series would itself be garbage for its own
    # warmup window) -- so rsi_period+wma_period, not max(). SMA50 and the
    # momentum-candle lookback window are separate, usually-smaller
    # requirements. +n_sustain_candles on top guarantees the sustain-
    # counter tracking has real history behind it from day 1.
    lookback = config.momentum_candle_lookback_k + config.momentum_candle_range_avg_window
    warmup_needed = max(
        config.rsi_period + config.wma_period,
        config.sma_period,
        config.trail_ma_period,
        lookback,
    ) + config.n_sustain_candles
    if len(bars) < warmup_needed:
        Log.warning(
            ...
            f"RSI{config.rsi_period}/WMA{config.wma_period}/SMA{config.sma_period}) -- no signal."
        )
        return None
    ...
    rsi_raw = np.asarray(ta.rsi(close, config.rsi_period))
    # ta.wma's Rust backend does NOT recover from a leading NaN in its
    # input -- confirmed empirically: once ANY NaN appears anywhere in the
    # input array, EVERY output position stays NaN forever, even bars far
    # past the NaN region (not the usual "not enough warmup yet" NaN
    # pattern the rest of this codebase is used to). RSI(rsi_period)'s own
    # output always has a NaN prefix of length rsi_period (its own
    # warmup), so a naive ta.wma(rsi_raw, wma_period) would make hm_line
    # permanently NaN on every single call -- caught via this script's own
    # test suite before deployment, not a hypothetical. Fix: compute WMA
    # only on the NaN-free tail of rsi_raw, then pad the result back to
    # the original length with NaN for the stripped prefix (so hm_line
    # stays index-aligned with rsi_raw/close/bars throughout).
    valid_from = int(np.argmax(~np.isnan(rsi_raw))) if np.any(~np.isnan(rsi_raw)) else len(rsi_raw)
    hm_line = np.full_like(rsi_raw, np.nan)
    if valid_from < len(rsi_raw):
        hm_line[valid_from:] = np.asarray(ta.wma(rsi_raw[valid_from:], config.wma_period))
    sma50 = np.asarray(ta.sma(close, config.sma_period))
    # 2026-08-15: EMA(trail_ma_period=20) -- the trailing EXIT reference,
    # replacing SMA50 there after real-data backtesting showed SMA50 lags
    # badly on fast reversals. SMA50 itself is untouched -- still used for
    # entry confirmation and the re-entry touch-zone gate.
    trail_ma = np.asarray(ta.ema(close, config.trail_ma_period))

    # hm_prev1/hm_prev2 both need to be real numbers for the threshold-cross
    # detection in _update_hm_state_machine -- confirm the warmup floor
    # above actually delivered that (it should always hold given
    # warmup_needed's rsi_period+wma_period term, but this is the load-
    # bearing check that would catch it if that assumption were ever wrong,
    # e.g. after a future config change).
    if (np.isnan(hm_line[-1]) or np.isnan(hm_line[-2]) or np.isnan(sma50[-1]) or np.isnan(sma50[-2])
            or np.isnan(trail_ma[-1])):
        Log.warning(f"[{inst.name}] hm_line/sma50/trail_ma still NaN after warmup floor -- no signal this cycle.")
        return None
    ...
    signal = InstrumentSignal(
        ...
        rsi_prev1=float(rsi_raw[-1]),
        hm_prev1=float(hm_line[-1]), hm_prev2=float(hm_line[-2]),
        sma50_prev1=float(sma50[-1]), sma50_prev2=float(sma50[-2]),
        trail_ma_prev1=float(trail_ma[-1]),
    ...
    if _last_logged_candle.get(inst.name) != candle_key:
        ...
        Log.info(
            ...
            f"rsi={signal.rsi_prev1:.2f} hm_line={signal.hm_prev1:.2f} sma50={signal.sma50_prev1:.2f} "
            f"ema{config.trail_ma_period}={signal.trail_ma_prev1:.2f} ltp={ltp:.2f}"
        )
    return signal


def find_momentum_candle(high: np.ndarray, low: np.ndarray, k: int, avg_window: int,
                          multiplier: float) -> Optional[dict]:
    """Objective proxy for the source spec's "momentum candle" -- described
    there only visually ("the larger, decisive candle that kicked off the
    move"), no rule given. This project's rule: scan the last `k` bars,
    return the MOST RECENT bar whose (high-low) range exceeds `multiplier`
    times the mean range of the `avg_window` bars immediately PRECEDING
    that k-bar window (the baseline deliberately excludes the window being
    tested -- a candle can't be judged an outlier against a baseline that
    includes itself). Returns None if fewer than k+avg_window bars are
    available, or if no bar in the window qualifies -- callers must treat
    None as "no valid stop-loss reference, do not enter" (see
    Config.momentum_candle_multiplier/_lookback_k/_range_avg_window's own
    comments; both the window sizes are NOT specified by the source spec)."""
    if len(high) < k + avg_window or len(low) < k + avg_window:
        return None
    baseline_high = high[-(k + avg_window):-k] if k > 0 else high[-avg_window:]
    baseline_low = low[-(k + avg_window):-k] if k > 0 else low[-avg_window:]
    avg_range = float(np.mean(baseline_high - baseline_low))
    if avg_range <= 0:
        return None
    window_high, window_low = high[-k:] if k > 0 else high[:0], low[-k:] if k > 0 else low[:0]
    if len(window_high) == 0:
        return None
    ranges = window_high - window_low
    qualifying = np.where(ranges > multiplier * avg_range)[0]
    if len(qualifying) == 0:
        return None
    idx = int(qualifying[-1])   # most recent qualifying bar in the window
    return {"high": float(window_high[idx]), "low": float(window_low[idx]),
            "offset_from_end": k - idx}


def _refresh_momentum_candle(leg: "LegState", signal: "InstrumentSignal", leg_key: str):
    """Re-run find_momentum_candle() against the CURRENT rolling window and
    overwrite leg.momentum_candle_high/low/key with the result. Called
    both at the crossing candle AND on every subsequent sustaining candle
    (2026-08-15 fix -- see _update_hm_state_machine's call sites).

    Originally this only ran ONCE, at the crossing candle, then stayed
    fixed for the rest of the sustain streak. Real-data backtesting (1-7
    Aug 2026, 45m/60m intervals) surfaced a live consequence: HM_line
    stayed below its threshold continuously for ~a month before the
    separate SMA50 2-close condition also lined up, so entry_condition
    didn't actually fire until sustain_counter had reached 118 -- and the
    momentum candle locked in at candle 0 of that streak was, by then,
    over 450 points away from the eventual entry price and functionally
    meaningless as a stop reference. Re-running this every sustaining
    candle keeps the stop level temporally anchored to NOW (a rolling
    k-bar window ending at the current candle), not to whenever the
    episode happened to start -- self-correcting as the episode ages,
    exactly like a normal indicator computed fresh each candle. If no
    candle currently qualifies, momentum_candle_high/low reset to 0.0 --
    entry_condition's existing `momentum_candle_high > 0` gate then
    correctly blocks entry until a genuinely fresh one reappears, same
    behavior as the original never-found case."""
    k = min(config.momentum_candle_lookback_k, len(signal.high))
    mc = find_momentum_candle(signal.high, signal.low, k=k,
                               avg_window=config.momentum_candle_range_avg_window,
                               multiplier=config.momentum_candle_multiplier)
    if mc is not None:
        leg.momentum_candle_high = mc["high"]
        leg.momentum_candle_low = mc["low"]
        leg.momentum_candle_key = signal.candle_key
    else:
        leg.momentum_candle_high = 0.0
        leg.momentum_candle_low = 0.0
        leg.momentum_candle_key = ""
        Log.warning(f"[{leg_key}] no momentum candle currently qualifies "
                    f"(multiplier={config.momentum_candle_multiplier}) -- entry blocked until one reappears.")
...
class StrategyEngine:
    def __init__(self, client, store: StateStore, env: Environment, price_stream: "PriceStream",
        ...
        # Idempotency guard for _update_hm_state_machine -- ensures the
        # HM/RSI/SMA50 state machine advances at most once per newly-closed
        # candle per leg, not on every 10s run_cycle tick. Engine-level,
        # NOT persisted -- losing it across a restart just means the
        # current candle gets re-evaluated once more, harmless (same
        # idempotency tolerance _last_logged_candle relies on elsewhere).
        self._hm_last_processed_candle: dict[str, str] = {}
        ...
        def _run():
            ...
            try:
                ...
                for leg in self.store.state.legs.values():
                    ...
                    # HM/RSI/SMA50 state machine -- a stale "extended"/
                    # sustaining episode from yesterday shouldn't carry into
                    # a new session (the candle history it was tracking is
                    # now a full day stale). trade_count reset above already
                    # means today's re-entry cap is fresh regardless.
                    leg.wait_for_reversal = False
                    leg.sustain_counter = 0
                    leg.signal_candle_high = 0.0
                    leg.signal_candle_low = 0.0
                    leg.signal_candle_key = ""
                    leg.signal_confirmed_by_close = False
                    leg.momentum_candle_high = 0.0
                    leg.momentum_candle_low = 0.0
                    leg.momentum_candle_key = ""
                    leg.awaiting_reentry_trigger = False
                self._hm_last_processed_candle.clear()
                self._save_state()
    ...
    def _update_hm_state_machine(self, leg_key: str, leg: LegState, signal: InstrumentSignal, option_type: str):
        """Advances the source spec's extended/wait-for-reversal/sustain-
        counter/false-signal-filter state machine by exactly one candle --
        called once per newly-closed candle (run_cycle's
        self._hm_last_processed_candle guard), and ONLY while this leg is
        flat (has_position False) -- once a position is open,
        leg.momentum_candle_high/low IS that trade's live stop-loss level;
        letting this method keep updating it while a position is open would
        silently move the stop out from under an open trade. See module
        docstring's Signal Rules section for the full prose spec this
        mirrors line by line. PE tracks the SHORT-side episode (HM_line
        extended above THRESHOLD, then crosses below); CE is the exact
        mirror (extended below, crosses above)."""
        if option_type == "PE":
            extended = signal.hm_prev1 > signal.rsi_prev1 and signal.hm_prev1 > config.threshold
            if extended and not leg.wait_for_reversal:
                leg.wait_for_reversal = True
                leg.sustain_counter = 0
                leg.signal_confirmed_by_close = False
                leg.awaiting_reentry_trigger = False   # fresh episode -- clears the re-entry gate (spec's "OR a fresh HM_line cross" trigger)
                Log.info(f"[{leg_key}] extended_up detected -- arming wait_for_reversal")
                return
            if not leg.wait_for_reversal:
                return
            if leg.sustain_counter == 0:
                crossed_below = signal.hm_prev2 >= config.threshold and signal.hm_prev1 < config.threshold
                if crossed_below:
                    leg.signal_candle_high = signal.high_prev1
                    leg.signal_candle_low = signal.low_prev1
                    leg.signal_candle_key = signal.candle_key
                    _refresh_momentum_candle(leg, signal, leg_key)
                    leg.sustain_counter = 1
                return
            # sustain_counter > 0: already crossed, now sustaining or reversing.
            if signal.hm_prev1 < config.threshold:
                if not leg.signal_confirmed_by_close and signal.close_prev1 < leg.signal_candle_low:
                    leg.signal_confirmed_by_close = True
                # Keep the stop level fresh every sustaining candle, not
                # just the crossing candle -- see _refresh_momentum_candle's
                # own docstring for why (a long sustain streak would
                # otherwise lock in a stop from whenever the episode
                # happened to start, arbitrarily far from the eventual
                # entry).
                _refresh_momentum_candle(leg, signal, leg_key)
                leg.sustain_counter += 1
            elif not leg.signal_confirmed_by_close:
                # False-signal filter (spec: "only trust the cross once
                # price closes beyond the high/low of the crossing
                # candle") -- HM_line reversed back above THRESHOLD before
                # any confirming close ever happened. Reset the episode.
                Log.info(f"[{leg_key}] False signal: HM_line reversed back above threshold "
                          f"before a confirming close -- resetting.")
                leg.wait_for_reversal = False
                leg.sustain_counter = 0
            # else: already confirmed by close -- a flicker back above
            # threshold doesn't undo a genuinely confirmed move; leave
            # sustain_counter/state as-is.
        else:  # CE -- exact mirror
            extended = signal.hm_prev1 < signal.rsi_prev1 and signal.hm_prev1 < config.threshold
            if extended and not leg.wait_for_reversal:
                leg.wait_for_reversal = True
                leg.sustain_counter = 0
                leg.signal_confirmed_by_close = False
                leg.awaiting_reentry_trigger = False
                Log.info(f"[{leg_key}] extended_down detected -- arming wait_for_reversal")
                return
            if not leg.wait_for_reversal:
                return
            if leg.sustain_counter == 0:
                crossed_above = signal.hm_prev2 <= config.threshold and signal.hm_prev1 > config.threshold
                if crossed_above:
                    leg.signal_candle_high = signal.high_prev1
                    leg.signal_candle_low = signal.low_prev1
                    leg.signal_candle_key = signal.candle_key
                ...
                return
            if signal.hm_prev1 > config.threshold:
                if not leg.signal_confirmed_by_close and signal.close_prev1 > leg.signal_candle_high:
                    leg.signal_confirmed_by_close = True
                _refresh_momentum_candle(leg, signal, leg_key)
                leg.sustain_counter += 1
            elif not leg.signal_confirmed_by_close:
                Log.info(f"[{leg_key}] False signal: HM_line reversed back below threshold "
                          f"before a confirming close -- resetting.")
                leg.wait_for_reversal = False
                leg.sustain_counter = 0

    def _check_stop_loss(self, leg_key: str, leg: LegState, futures_underlying_px: Optional[float]
                          ) -> tuple[bool, Optional[float]]:
        """Momentum-candle stop -- a FIXED underlying-FUTURES price level,
        NOT a %-of-premium ratchet (contrast the EMA34/RSI/ADX sibling's
        compute_trailing_sl_price -- that mechanism is not used here, this
        strategy has no equivalent function). Reads the futures/underlying
        price (the SAME value run_cycle already fetches for the signal
        path -- WS-cache-first, REST-fallback, passed in here rather than
        re-fetched) and compares against leg.momentum_candle_high (PE) /
        leg.momentum_candle_low (CE), locked in once at entry by
        find_momentum_candle(). Returns (False, None) if no trustworthy
        underlying price is available this cycle, or if this leg never had
        a momentum candle locked in (nothing to check against) -- never
        acts on a missing/stale quote, same philosophy as the rest of this
        file. Never places an order itself -- run_cycle's existing exit
        dispatch handles that, same as every other exit reason."""
        if futures_underlying_px is None:
            return False, None
        option_type = leg_key.split("_")[1]
        if option_type == "PE":
            if leg.momentum_candle_high <= 0:
                return False, None
            return futures_underlying_px > leg.momentum_candle_high, leg.momentum_candle_high
        else:
            if leg.momentum_candle_low <= 0:
                return False, None
            return futures_underlying_px < leg.momentum_candle_low, leg.momentum_candle_low
    ...
    def _finalize_exit(self, leg_key: str, inst: InstrumentConfig, reason: str):
        ...
        # over a fresh quote (2026-08-18: this priority tier didn't exist
        # before -- see docs/CUSTOMIZATIONS.md's "recurring gotcha" note).
        # WS-cache/REST-quote fallback only applies to the "manual"
        # resolution path or the rare case poll_fill's response carried
        # neither average_price nor price.
        ...
        try:
            ...
            for inst in INSTRUMENTS:
                ...
                for option_type in ("PE", "CE"):
                    ...
                    if not has_position:
                        # Advance the HM/RSI/SMA50 state machine at most once
                        # per newly-closed candle -- see _update_hm_state_machine's
                        # own docstring for why this is skipped entirely while a
                        # position is open (it would corrupt the live SL level).
                        if self._hm_last_processed_candle.get(leg_key) != signal.candle_key:
                            self._hm_last_processed_candle[leg_key] = signal.candle_key
                            self._update_hm_state_machine(leg_key, leg, signal, option_type)

                    # 2026-08-15: exit trails EMA(trail_ma_period=20), not
                    # SMA50 -- SMA50 stays entry-only (2-close confirmation)
                    # + the re-entry touch-zone gate below. See
                    # Config.trail_ma_period's own comment for why.
                    if option_type == "CE":
                        exit_condition = signal.close_prev1 < signal.trail_ma_prev1
                    else:
                        exit_condition = signal.close_prev1 > signal.trail_ma_prev1
                    ...
                    if has_position:
                        ...
                        # LATER cycle (e.g. price recovers before the async fill lands)
                        ...
                        # Momentum-candle stop -- checked every cycle using the
                        # LIVE futures/underlying price, independent of the
                        # candle_interval_minutes candle gating that governs
                        # exit_condition above (see _check_stop_loss's own docstring).
                        sl_hit, sl_price = self._check_stop_loss(leg_key, leg, inst_ltp)
                        if exit_condition or sl_hit or exit_already_committed:
                            if not exit_already_committed:
                                if sl_hit and not exit_condition:
                                    Log.info(f"[{leg_key}] Momentum-candle stop hit -> closing. "
                                              f"futures_ltp vs sl_price={sl_price:.2f}")
                                elif exit_condition:
                                    Log.info(f"[{leg_key}] SMA50 trailing exit -> closing.")
                            reason = "momentum_candle_stop" if (sl_hit and not exit_condition) else "sma50_trail_exit"
                            # Blocks a fresh entry on THIS SAME leg until the
                            # WALL-CLOCK candle advances -- see LegState.
                            # last_sl_exit_candle_boundary's own comment
                            # (inherited from the EMA34/RSI/ADX sibling) for why
                            # this is _current_candle_boundary(), not
                            # signal.candle_key. Applied on ANY exit here (not
                            # just the momentum-candle stop) -- also blocks an
                            # immediate re-entry after an SMA50 trailing exit on
                            # the exact same stale-candle reading.
                            leg.last_sl_exit_candle_boundary = str(
                                _current_candle_boundary(config.candle_interval_minutes)
                            )
                            # Re-entry gate (spec: multiple re-entries per
                            # session allowed) -- cleared later either by a
                            # fresh extended->crossed episode arming
                            # (_update_hm_state_machine) or an SMA50-zone touch
                            # (checked below, once flat).
                            leg.awaiting_reentry_trigger = True
                    ...
                    if leg.last_sl_exit_candle_boundary and leg.last_sl_exit_candle_boundary == str(
                        ...
                        continue
                    if leg.awaiting_reentry_trigger:
                        # Gate clears here on an SMA50-zone touch; a fresh
                        # extended->crossed episode already cleared it inside
                        # _update_hm_state_machine above (same cycle, before this
                        # check runs) -- whichever trigger fires first per spec.
                        touched_sma50 = (
                            signal.sma50_prev1 > 0
                            and abs(signal.close_prev1 - signal.sma50_prev1) / signal.sma50_prev1
                            <= config.sma50_touch_zone_pct
                        )
                        if touched_sma50:
                            leg.awaiting_reentry_trigger = False
                        else:
                    ...
                    if option_type == "CE":
                        entry_condition = (
                            leg.sustain_counter >= config.n_sustain_candles
                            and leg.signal_confirmed_by_close
                            and signal.close_prev1 > signal.sma50_prev1
                            and signal.close_prev2 > signal.sma50_prev2   # spec's explicit 2-close confirmation
                            and leg.momentum_candle_low > 0               # a momentum candle was found -- see find_momentum_candle
                        )
                    else:  # PE
                        entry_condition = (
                            leg.sustain_counter >= config.n_sustain_candles
                            and leg.signal_confirmed_by_close
                            and signal.close_prev1 < signal.sma50_prev1
                            and signal.close_prev2 < signal.sma50_prev2
                            and leg.momentum_candle_high > 0
                        )
                    ...
                    if option_type == "CE":
                        condition_desc = (
                            f"sustain_counter={leg.sustain_counter} >= n_sustain_candles={config.n_sustain_candles}, "
                            f"signal_confirmed_by_close={leg.signal_confirmed_by_close}, "
                            f"close_prev1={signal.close_prev1:.2f} > sma50_prev1={signal.sma50_prev1:.2f}, "
                            f"close_prev2={signal.close_prev2:.2f} > sma50_prev2={signal.sma50_prev2:.2f}, "
                            f"momentum_candle_low={leg.momentum_candle_low:.2f}"
                        )
                    else:
                        condition_desc = (
                            f"sustain_counter={leg.sustain_counter} >= n_sustain_candles={config.n_sustain_candles}, "
                            ...
                            f"close_prev1={signal.close_prev1:.2f} < sma50_prev1={signal.sma50_prev1:.2f}, "
                            f"close_prev2={signal.close_prev2:.2f} < sma50_prev2={signal.sma50_prev2:.2f}, "
                            f"momentum_candle_high={leg.momentum_candle_high:.2f}"
                        )
                    self._enter_leg(leg_key, inst, option_type, spot=signal.ltp, futures_symbol=futures_symbol,
                                     condition_desc=condition_desc)
                    # Reset the state machine on successful entry dispatch
                    # (spec: "reset wait_for_reversal") -- _enter_leg sets
                    # pos.symbol synchronously before returning (only the FILL
                    # wait is backgrounded, see place()'s own docstring), so
                    # has_position will already be True next cycle regardless;
                    # this reset is about the SIGNAL episode ending, not order
                    # dedup.
                    leg.wait_for_reversal = False
                    leg.sustain_counter = 0
...
def print_banner():
    ...
    print(f"RSI / WMA(HM) / SMA  : {config.rsi_period} / {config.wma_period} / {config.sma_period} (threshold {config.threshold})")
    print(f"Interval             : {config.candle_interval_minutes}m (resampled from {config.fetch_interval} fetch)")
    print(f"Sustain candles      : {config.n_sustain_candles} (needs backtesting)")
    print(f"Momentum candle      : {config.momentum_candle_multiplier}x avg range, "
          f"lookback {config.momentum_candle_lookback_k}, avg window {config.momentum_candle_range_avg_window} "
          f"(needs backtesting)")
    print(f"SMA50 re-entry zone  : {config.sma50_touch_zone_pct:.2%} (needs backtesting)")
    print(f"Trailing exit MA     : EMA{config.trail_ma_period} (replaces SMA50 for exit only, 2026-08-15)")
    ...
    print("Stop-loss            : momentum-candle price level (fixed, on underlying futures) "
          "+ SMA50 trailing cross (see module docstring)")
