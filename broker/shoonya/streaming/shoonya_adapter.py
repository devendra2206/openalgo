            ...
            if scrip not in self._scrip_to_cids:
                ...
                if scrips_for_token is not None:
                    ...
                    if not scrips_for_token:
                        ...
                        # 2026-07-30: clear alongside, so a genuinely fresh
                        # future subscribe doesn't inherit a stale baseline
                        # from this unrelated earlier occupancy. Gated on the
                        # token having NO remaining scrips at all (not just
                        # this one) -- a token shared across exchanges (the
                        # exact collision issue #1732 scrip-keying exists to
                        # prevent) must keep its staleness tracking alive for
                        # whichever scrip still uses it.
