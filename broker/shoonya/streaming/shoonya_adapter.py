    ...
    def _remove_subscription_locked(
        ...
        if scrips_for_token is not None:
            ...
            if not scrips_for_token:
                ...
                # unrelated earlier occupancy. Gated on the token having NO
                # remaining scrips at all (not just this one) -- a token
                # shared across exchanges (the exact collision issue #1732
                # scrip-keying exists to prevent) must keep its staleness
                # tracking alive for whichever scrip still uses it.
