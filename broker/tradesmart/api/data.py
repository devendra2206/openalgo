...
class BrokerData:
    ...
    def get_multiquotes(self, symbols: list) -> list:
        ...
        # Sized to the shared per-second budget: threads are spawned lazily,
        # so a short list still costs only len(pending) threads. apply_rate_limit()
        # inside each fetch paces actual dispatch against the real ceiling.
        max_workers = min(TRADESMART_MAX_PER_SECOND, len(pending))
